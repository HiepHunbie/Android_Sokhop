...
public class OtherRecruitmentAdapter extends BaseAdapter {
    ...
    public OtherRecruitmentAdapter(Activity a, ArrayList<String> pos, ArrayList<String> places, ArrayList<String> typeWork, ArrayList<String> timeEdit,
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        View vi=convertView;
        if(convertView==null)
            vi = inflater.inflate(R.layout.item_other_recruitment, null);
        ...
        if(timeEdit.get(position).length()>5) {
            txtTimeSuaLanCuoi.setText(DateUtils.convertDateFormat(timeEdit.get(position).substring(0, 10)));
        }
        ...
        if(ngayhethan.get(position).length()>5){
            txtTimeHetHan.setText(DateUtils.convertDateFormat(ngayhethan.get(position).substring(0,10)));
        }
        return vi;
    }
}
