...
public class NotificationTabFragment extends BaseFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.tab_info_1, container, false);

        mActivity.databaseHelper.deleteAllNotification();
        notificationModels = new ArrayList<>();
        getNotification(mActivity.p.getString(Pref.PREF_KEY_TOKEN,null),"1","1","10");
        listView = (SwipeMenuListView)rootView.findViewById(R.id.listView);
        customList = new TabInfoAdapter(mActivity, names, messages, times,quantam,isRead,notify_ids,0);
        ...
        SwipeMenuCreator creator = new SwipeMenuCreator() {
            ...
            public void create(SwipeMenu menu) {
                ...
                SwipeMenuItem spam = new SwipeMenuItem(
                        mActivity);
                spam.setBackground(R.color.recruitment1);
                spam.setWidth((int) mActivity.getResources().getDimension(R.dimen._80sdp));
                spam.setTitleSize(20);
                spam.setTitleColor(Color.WHITE);
                spam.setTitle(getString(R.string.spam));
                menu.addMenuItem(spam);
            ...
            private void isScrollCompleted() {
                ...
                if (currentFirstVisibleItem + currentVisibleItemCount >= currentTotalItemCount) {
                    if (this.currentVisibleItemCount > 0
                        ...
                        refreshValue+=1;
                        getNotification(mActivity.p.getString(Pref.PREF_KEY_TOKEN,null),"1",String.valueOf(refreshValue),"10");
                    }
                }
            }
        });
        listView.setOnMenuItemClickListener(new SwipeMenuListView.OnMenuItemClickListener() {
            ...
            public boolean onMenuItemClick(int position, SwipeMenu menu, int index) {
                switch (index) {
                    case 0:
                        int response = HttpUpdateNotification.updateTypeNotification(mActivity.p.getString(Pref.PREF_KEY_TOKEN,null),notify_ids.get(position),"3");
                        if(response == 200){
                            DialogCall.showResponse(mActivity, mActivity.getString(R.string.spam_success), new View.OnClickListener() {
                                @Override
                                public void onClick(View v) {
                                    int id = v.getId();
                                    if (id == R.id.btnOk) {
                                        InfoFragment.selectItem(0);
                                    }
                                }
                            });
                        }else {
                            DialogCall.showResponse(mActivity, mActivity.getString(R.string.spam_error), new View.OnClickListener() {
                                @Override
                                public void onClick(View v) {
                                    int id = v.getId();
                                    if (id == R.id.btnOk) {
