...
public class PublishedRecruitmentFragment extends BaseFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.fragment_published_recruitment, container, false);
        mActivity.databaseHelper.deleteAllRecruitment();
        recruitmentModels = new ArrayList<>();
        getRecruitment(mActivity.p.getString(Pref.PREF_KEY_TOKEN,null),0,"2","1","10");
        ...
        SwipeMenuCreator creator = new SwipeMenuCreator() {
            ...
            public void create(SwipeMenu menu) {
                ...
                SwipeMenuItem lock = new SwipeMenuItem(
                        mActivity);
                lock.setBackground(R.color.recruitment2);
                lock.setWidth((int) mActivity.getResources().getDimension(R.dimen._80sdp));
                lock.setTitleSize(15);
                lock.setTitleColor(Color.WHITE);
                lock.setTitle(getString(R.string.lock));
        ...
        listView.setOnMenuItemClickListener(new SwipeMenuListView.OnMenuItemClickListener() {
            ...
            public boolean onMenuItemClick(int position, SwipeMenu menu, int index) {
                switch (index) {
                    case 0:
                        mActivity.p.putString(Pref.PREF_KEY_JOB_ID,JobId.get(position));
                        mActivity.p.putString(Pref.PREF_KEY_STATUS_ID,"2");
                        mActivity.selectItem(18);
                        break;
                    case 1:
                        int response = HttpUpdateStatusRecruitment.updateStatusRecruitment(mActivity.p.getString(Pref.PREF_KEY_TOKEN,null),JobId.get(position),"3",mActivity);
                        if(response == 200){
                            DialogCall.showResponse(mActivity, mActivity.getString(R.string.dakhoa_success), new View.OnClickListener() {
                                @Override
                                public void onClick(View v) {
                                    int id = v.getId();
                                    if (id == R.id.btnOk) {
                                        QuanLyTinTuyenDungFragment.selectItem(1);
                                    }
                                }
                            });
                        }else {
                            DialogCall.showResponse(mActivity, mActivity.getString(R.string.dakhoa_error), new View.OnClickListener() {
                                @Override
                                public void onClick(View v) {
                                    int id = v.getId();
                                    if (id == R.id.btnOk) {
            ...
            private void isScrollCompleted() {
                ...
                if (currentFirstVisibleItem + currentVisibleItemCount >= currentTotalItemCount) {
                    if (this.currentVisibleItemCount > 0
                        ...
                        refreshValue+=1;
                        getRecruitment(mActivity.p.getString(Pref.PREF_KEY_TOKEN,null),0,"2", String.valueOf(refreshValue),"10");
                    }
                }
            }
        });
...
//                            String job_location = jsonObject.getString("");
                        ...
                        recruitmentModels = mActivity.databaseHelper.getARecruitment(2);
